...
import com.aura.gmall.realtime.bean.{SpuInfo, TradeMark}
...
object TradeMarkApp {
    def main(args: Array[String]): Unit = {
        ...
        //加载偏移量起始位置
        val topicName = "ODS_BASE_TRADEMARK"
        val groupId = "TRADEMARK_GROUP"
        ...
        //封装为对象
        val tradeMarkDStream: DStream[TradeMark] = recordDStream.map(
            record => {
                val jsonStr: String = record.value()
                val tradeMark: TradeMark = JSON.parseObject(jsonStr, classOf[TradeMark])
                tradeMark
            }
        )
        //存入HBase
        tradeMarkDStream.foreachRDD(
            rdd => {
                import org.apache.phoenix.spark._
                rdd.saveToPhoenix("BASE_TRADEMARK0105",
                    Seq("TM_ID", "TM_NAME"),
                    new Configuration,
